...
function DateBadge({ date }) {
  return (
    <span style={{
      ...
      background: '#e0f2fe', color: '#0369a1', fontFamily: 'monospace',
    }}>
      {date}
    </span>
  );
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{
          padding: '4px 12px', borderRadius: 6, fontSize: 15, fontWeight: 700,
      ...
      {open && (
        <div style={{
          ...
          background: '#fff', borderRadius: 8, minWidth: 165,
...
function renderCell(col, row, idx, role) {
  switch (col.key) {
    ...
    case 'actions':
      return role === 'admin'
        ? <ActionsDropdown actions={SERVICE_ACTIONS.admin} />
        : (
...
export default function Services({ role = 'admin' }) {
  // In production, role comes from App.jsx — remove this line and use the prop directly
  ...
  return (
    <div style={{ padding: '24px 28px', fontFamily: 'Roboto, sans-serif', background: '#f8fafc', minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        {/* Role switcher — only visible to admin, remove in production */}
        {activeRole === 'admin' && (
            ...
            {['admin', 'partner'].map(r => (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${activeRole === r ? '#3b82f6' : '#e2e8f0'}`,
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {[
          { label: 'Total Services', value: svcRows.length,                                      color: '#3b82f6' },
          { label: 'Active',         value: svcRows.filter(r => r.status === 'ACTIVE').length,   color: '#16a34a' },
          { label: 'Inactive',       value: svcRows.filter(r => r.status === 'INACTIVE').length, color: '#dc2626' },
        ].map(s => (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {svcRows.map((row, idx) => (
                <tr
                  key={row.id}
                  style={{ background: '#fff', borderBottom: '1px solid #f1f5f9', transition: 'background 0.1s' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'}
