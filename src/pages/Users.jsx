...
const USER_TYPES = ['All', 'Admins', 'C-Admins', 'Supper Clients', 'Clients', 'Client Partner', 'Sub Account', 'Publisher', 'Mirrored', 'Operations'];
...
const PARTNER_SUB_ACCOUNTS = [
  {
    id: 'TDG4240', name: 'TDG4240', type: 'SP Account For Details',
    lastLogin: '- -', lastAccessed: '- -', status: true,
    services: [
      'Noble Horo - 4240002 - True',    'Horo Mine - 4240003 - True',
      'Horo Majestic - 4240004 - True', 'Majestic 2 Horo - 4240005 - True',
      'Horo Symbolic - 4240006 - True', 'Horo Habits - 4240007 - True',
      'Horo Veiled - 4240008 - True',   'Horosard DD - 4240009 - True',
      'Taro Horo - 4240010 - True',     'Super Wow Horo - 4240011 - True',
      'Good Health D - 4240012 - True', 'Health Happy - 4240013 - True',
      'Morning Health - 4240014 - True','Hi Healthy - 4240015 - True',
      'Healthy Fine - 4240016 - True',  'Healthy Mine - 4240017 - True',
      'Healthy Eat - 4240018 - True',   'Healthy Habits - 4240019 - True',
      'Healthy Meals - 4240020 - True', 'Healthy Style - 4240021 - True',
    ],
  },
  {
    id: 'CLT8821', name: 'CLT8821', type: 'Client Account',
    lastLogin: 'Feb 24, 11:30', lastAccessed: '192.168.1.44', status: true,
    services: [
      'Shield Core - 8821001 - True', 'Click Pro - 8821002 - True',
      'APK Scan Lite - 8821003 - True', 'Fraud Guard - 8821004 - True',
    ],
  },
  {
    id: 'PRT3310', name: 'PRT3310', type: 'Sub Account',
    lastLogin: 'Feb 23, 09:12', lastAccessed: '10.0.0.15', status: false,
    services: [
      'Basic Tracker - 3310001 - False', 'Geo Resolver - 3310002 - True',
    ],
  },
  {
    id: 'AGT9902', name: 'AGT9902', type: 'Agent Account',
    lastLogin: 'Today 08:55', lastAccessed: '89.46.20.11', status: true,
    services: [
      'Premium Shield - 9902001 - True', 'APK Vault - 9902002 - True',
      'Network Guard - 9902003 - True',  'DataSync - 9902004 - True',
  ...
  return (
    <div ref={ref} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      {PRIMARY_ACTIONS.map((a) => (
        <button key={a.key} title={a.title} style={{
          width: 30, height: 30, borderRadius: 8, border: 'none', cursor: 'pointer',
          background: `${a.color}15`, color: a.color, fontSize: 13,
      ...
      {/* Block / Unblock */}
      <button title={isBlocked ? 'Unblock' : 'Block'} style={{
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setOpen((o) => !o)} title="More actions" style={{
        ...
        {open && (
          <div style={{
            ...
            background: '#fff', borderRadius: 12, border: '1px solid #e8ecf3',
            boxShadow: '0 12px 36px rgba(0,0,0,.14)', minWidth: 200, padding: '6px 0',
          }}>
            {/* Header */}
            <div style={{ padding: '6px 14px 8px', borderBottom: '1px solid #f1f5f9', marginBottom: 4 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#1a1a2e' }}>{user.name}</div>
              <div style={{ fontSize: 10, color: '#94a3b8' }}>{user.email}</div>
            </div>

            {MORE_ACTIONS.map((a) => (
              <button key={a.key} onClick={() => setOpen(false)} style={{
            ...
            >
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: '#dc2626' }}>Delete User</span>
            </button>
          </div>
        )}
      </div>
...
function ActiveInactiveTabs({ value, onChange, activeCount, inactiveCount }) {
  const tabs = [
    { key: 'active',   label: 'Active',   count: activeCount,   color: '#16a34a', bg: '#dcfce7', dot: '#22c55e' },
    { key: 'inactive', label: 'Inactive', count: inactiveCount, color: '#d97706', bg: '#fef3c7', dot: '#f59e0b' },
  ];
  return (
      ...
      {tabs.map(({ key, label, count, color, bg, dot }) => {
        ...
        return (
          <button key={key} onClick={() => onChange(key)} style={{
            ...
            borderBottom: on ? `2.5px solid ${color}` : '2.5px solid transparent',
            marginBottom: -2,
            transition: 'all .15s',
          }}>
            <span style={{
...
export default function PageUsers({ role = 'admin' }) {
  ...
  const filtered = userRows.filter((u) => {
    const matchStatus = statusTab === 'active' ? isActiveStatus(u) : isInactiveStatus(u);
  ...
  const toggleAccount = (id) => setExpandedAccounts((p) => ({ ...p, [id]: !p[id] }));
  ...
  const activePartnerCount   = PARTNER_SUB_ACCOUNTS.filter((u) => u.status === true).length;
  const inactivePartnerCount = PARTNER_SUB_ACCOUNTS.filter((u) => u.status === false).length;

  if (isPartner) {
    return (
      <div>
        {/* Header */}
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e8ecf3', marginBottom: 16, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px' }}>
            <div>
              <div style={{ fontSize: 16, fontWeight: 800, color: '#1a1a2e' }}>My Sub-Accounts</div>
              <div style={{ fontSize: 12, color: SLATE, marginTop: 2 }}>{PARTNER_SUB_ACCOUNTS.length} accounts · {PARTNER_SUB_ACCOUNTS.reduce((s,u)=>s+u.services.length,0)} total services</div>
            </div>
            <button style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filteredPartnerAccounts.length === 0 && (
            <div style={{ textAlign: 'center', padding: 40, color: SLATE, fontSize: 13, background: '#fff', borderRadius: 14, border: '1px solid #e8ecf3' }}>
          ...
          {filteredPartnerAccounts.map((u) => {
            ...
            return (
              <div key={u.id} style={{
                ...
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: '200px 200px 160px 180px 80px 1fr auto',
                  alignItems: 'center',
                  padding: '0',
                  ...
                  <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{
                      ...
                      background: u.status ? 'linear-gradient(135deg,#0d9488,#0891b2)' : '#f1f5f9',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 13, fontWeight: 800, color: u.status ? '#fff' : '#94a3b8',
                    }}>{u.name.slice(0,3)}</div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 800, color: '#1a1a2e' }}>{u.name}</div>
                      <div style={{ fontSize: 10, color: SLATE, marginTop: 1 }}>ID: {u.id}</div>
                    </div>
                  </div>
                  ...
                  {/* Last login */}
                  <div style={{ padding: '16px 12px' }}>
                    <div style={{ fontSize: 11, color: SLATE, marginBottom: 2 }}>Last Login</div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#334155' }}>{u.lastLogin}</div>
                  </div>

                  {/* Last accessed */}
                  <div style={{ padding: '16px 12px' }}>
                    <div style={{ fontSize: 11, color: SLATE, marginBottom: 2 }}>Last Accessed From</div>
                    <div style={{ fontSize: 12, fontFamily: 'monospace', color: '#334155' }}>{u.lastAccessed}</div>
                  </div>
                  ...
                  <div style={{ padding: '16px 12px' }}>
                    <div style={{
                      ...
                      <div style={{
                        ...
                        left: u.status ? 23 : 3, width: 18, height: 18,
                  ...
                  <div style={{ padding: '14px 12px', display: 'flex', flexWrap: 'wrap', gap: 5, alignItems: 'center' }}>
                    {preview.map((svc) => (
                      <span key={svc} style={{
                        padding: '3px 9px', borderRadius: 6, fontSize: 10, fontWeight: 600,
                        background: '#0d9488', color: '#fff', whiteSpace: 'nowrap',
                      }}>{svc}</span>
                    ))}
                    {rest.length > 0 && (
                      <button onClick={() => toggleAccount(u.id)} style={{
                        padding: '3px 10px', borderRadius: 6, fontSize: 10, fontWeight: 700,
                        border: '1.5px dashed #0d9488', background: 'transparent',
                        color: '#0d9488', cursor: 'pointer', whiteSpace: 'nowrap',
                      }}>
                  ...
                  {/* Actions */}
                  <div style={{ padding: '16px 18px', display: 'flex', gap: 6 }}>
                    <button title="View" style={{
                      width: 30, height: 30, borderRadius: 8, border: 'none', cursor: 'pointer',
                      background: '#f0fdfa', color: '#0d9488', fontSize: 14,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>👁</button>
                ...
                {isExpanded && (
                  <div style={{ padding: '14px 20px 16px', background: '#f8fafc' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: SLATE, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.6px' }}>
                      ...
                      {u.services.map((svc) => {
                        ...
                        return (
                          <div key={svc} style={{
                            ...
                            background: '#fff', border: `1px solid ${isTrue ? '#99f6e4' : '#fecaca'}`,
                            color: '#334155',
                          }}>
                            <span>{svc.replace(/ - (True|False)$/, '')}</span>
                            <span style={{
                              padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700,
                              background: isTrue ? '#0d9488' : '#dc2626', color: '#fff', marginLeft: 8, flexShrink: 0,
                            }}>{isTrue ? 'True' : 'False'}</span>
                          </div>
                        );
                      })}
                    </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
        {[
          ...
          { label: 'Active',      value: userRows.filter(u => u.status === 'active').length,  color: GREEN },
          { label: 'Inactive',    value: userRows.filter(u => u.status === 'warning').length, color: AMBER },
          { label: 'Blocked',     value: userRows.filter(u => u.status === 'blocked').length, color: ROSE  },
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={repTrend}>
              ...
              <Line dataKey="clicks" stroke="#22c55e" strokeWidth={2} strokeDasharray="4 2" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ position: 'relative', flexShrink: 0 }}>
              <PieChart width={110} height={110}>
                <Pie data={TYPE_COUNTS} dataKey="count" cx={52} cy={52}
                  innerRadius={28} outerRadius={50} paddingAngle={2}>
                  {TYPE_COUNTS.map((t, i) => <Cell key={i} fill={t.color} />)}
                </Pie>
              </PieChart>
              <div style={{
                position: 'absolute', top: '50%', left: '50%',
                transform: 'translate(-50%,-50%)',
                fontSize: 14, fontWeight: 900, color: '#1a1a2e', textAlign: 'center', lineHeight: 1,
              }}>{TOTAL_USERS}</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: 1, overflow: 'hidden' }}>
              {TYPE_COUNTS.map((t) => (
                <div key={t.label} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#1a1a2e', whiteSpace: 'nowrap' }}>{t.count}</span>
                  <span style={{ fontSize: 10, color: SLATE, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <input placeholder="Search..." value={query} onChange={(e) => setQuery(e.target.value)} style={{
        ...
        <ActiveInactiveTabs
          value={statusTab}
          onChange={(v) => { setStatusTab(v); setActiveType('All'); }}
          activeCount={activeUsersCount}
          inactiveCount={inactiveUsersCount}
        />

        {/* Type filter tabs */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 16, marginBottom: 16, paddingBottom: 14, borderBottom: '1px solid #f1f5f9' }}>
          {USER_TYPES.map((t) => {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
              {['User', 'Email', 'Type', 'Region', 'Sessions', 'Last Login', 'Status', 'Actions'].map((h) => (
          ...
          <tbody>
            {filtered.map((u, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}
                ...
                <td style={{ padding: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      background: TYPE_COLORS[u.role] ? `${TYPE_COLORS[u.role]}22` : '#e2e8f0',
                      border: `2px solid ${TYPE_COLORS[u.role] || '#cbd5e1'}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontWeight: 800, color: TYPE_COLORS[u.role] || '#334155', fontSize: 13,
                    }}>{u.name[0]}</div>
                ...
                <td style={{ padding: 10 }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: STATUS_COLOR(u.status), display: 'inline-block', marginRight: 6 }} />
                  <span style={{ fontWeight: 600, textTransform: 'capitalize' }}>{u.status}</span>
                </td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={8} style={{ padding: 30, textAlign: 'center', color: SLATE, fontSize: 13 }}>
                  No {statusTab} users found{activeType !== 'All' ? ` for ${activeType}` : ''}.
                </td>
              </tr>
            )}
          </tbody>
